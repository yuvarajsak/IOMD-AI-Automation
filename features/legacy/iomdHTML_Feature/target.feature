	Feature: I2i_Validates autofill functionality in Target website

  @iomd_htmlTest @all
  Scenario Outline: TCs User validate autofill functionality on signup in Target
    ...
    Examples: 
      | url                                   | Signup  |
      | https://iomdqaenv.web.app/target.html | Sign Up |

  @iomd_html @all
  Scenario Outline: TC_01 User validate autofill functionality on signin form in Target
    When user completes onboarding flow
    ...
    Examples: 
      | url                                   | SignIn  |
      | https://iomdqaenv.web.app/target.html | Sign In |

  @iomd_html @all
  Scenario Outline: TC_03 User validate autofill functionality on forgotpassword form in Target
    When user completes onboarding flow
    When user navigates to the settings application and enables the extension
    Then user navigates to the safari application
    When user launches the website "<url>"
    ...
    Examples: 
      | url                                   | Forgotpassword  |
      | https://iomdqaenv.web.app/target.html | Forgot Password |

  @iomd_html1 @all
  Scenario Outline: TC_04 User validate autofill functionality on Checkout and Payment in Target
    ...
    Examples: 
      | url                                   | checkout             |
      | https://iomdqaenv.web.app/target.html | Checkout And Payment |

  @iomd_html1 @all
  Scenario Outline: TC_04 User validate autofill functionality on Redcard Apply in Target
    When user completes onboarding flow with card data
    When user launches the website "<url>"
    Then user clicks "<redcard>" button by only name in the Website
    When user enables the Extension
    Then user selects personal Username from Extension
    And user selects personal Address from Extension
    And user selects personal Phonenumber from Extension
    ...
    Examples: 
      | url                                   | redcard       |
      | https://iomdqaenv.web.app/target.html | Redcard Apply |
