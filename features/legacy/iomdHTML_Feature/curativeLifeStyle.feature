Feature: I2i_Validates autofill functionality in CurativeLifeStyle website

  @iomd_htmlTest @all_html
  Scenario Outline: TCs User validate autofill functionality in CurativeLifeStyle
    When user completes onboarding flow with card data
    When user launches the website "<url>"
    Then user clicks "<loginwithpassword>" button by only name in the Website
    ...
    Examples: 
      | url                                              | loginwithpassword   | checkout | payment |
      | https://iomdqaenv.web.app/curativeLifestyle.html | Login With Password | Checkout | Payment |

  @iomd_html @all
  Scenario Outline: TC_01 User validate autofill functionality on signin form in CurativeLifeStyle
    ...
    Examples: 
      ...
      | https://iomdqaenv.web.app/curativeLifestyle.html | Login With Password |

  @iomd_html @all
  Scenario Outline: TC_02 User validate autofill functionality on checkout form in CurativeLifeStyle
    When user completes onboarding flow
    When user navigates to the settings application and enables the extension
    Then user navigates to the safari application
    When user launches the website "<url>"
    ...
    Examples: 
      | url                                              | checkout |
      | https://iomdqaenv.web.app/curativeLifestyle.html | Checkout |

  @iomd_html @all
  Scenario Outline: TC_03 User validate autofill functionality on payment form in CurativeLifeStyle
    When user completes onboarding flow with card data
    When user launches the website "<url>"
    Then user clicks "<payment>" button by only name in the Website
    When user enables the Extension
    ...
    Examples: 
      | url                                              | payment |
      | https://iomdqaenv.web.app/curativeLifestyle.html | Payment |
