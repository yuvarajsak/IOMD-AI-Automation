Feature: I2i_Validates autofill functionality in Expedia website

  @iomd_html @all @expedia_html
  Scenario Outline: TC_01 User validate autofill functionality on signin form in Expedia
    ...
    Examples: 
      | url                                    | SignIn  |
      | https://iomdqaenv.web.app/expedia.html | Sign In |

  @iomd_html @all @expedia_html
  Scenario Outline: TC_02 User validate autofill functionality on signup form in Expedia
    When user completes onboarding flow
    When user navigates to the settings application and enables the extension
    Then user navigates to the safari application
    When user launches the website "<url>"
    ...
    Examples: 
      | url                                    | Signup  |
      | https://iomdqaenv.web.app/expedia.html | Sign Up |

  @iomd_html @all @expedia_html
  Scenario Outline: TC_04 User validate autofill functionality on checkout form in Expedia
    ...
    Examples: 
      | url                                    | checkout             |
      | https://iomdqaenv.web.app/expedia.html | Checkout And Payment |
